...
public sealed class SampleVisibilityResolver : IVisibilityResolver
{
    private const float TransitionMinPixels = 1.5f;
    private const int TransitionMaxDepth = 8;

    public IReadOnlyList<VisibilitySegment> Resolve(NprContext context, IReadOnlyList<FeatureCurve> curves)
    {
        var segments = new List<VisibilitySegment>(curves.Count * 2);
    ...
    private static void ResolveCurve(NprContext context, FeatureCurve curve, List<VisibilitySegment> segments)
    {
        ...
        var sampleCount = Math.Max(3, (int)MathF.Ceiling(lineLength / 6f) + 1);
        ...
        for (var index = 0; index < sampleCount - 1; index++)
        {
            ...
            var transition = RefineTransition(
                context,
                line.Depth,
                startT,
                endT,
    ...
    private static VisibilitySegment CreateSegment(
        ...
        float depth)
    {
        var kind = curve.Kind == FeatureCurveKind.Silhouette && state == VisibilityState.Visible
            ? FeatureCurveKind.OccludingContour
            : state == VisibilityState.Hidden && IsConstructionEligible(curve.Kind)
            ? FeatureCurveKind.Construction
            : curve.Kind;
        var intent = kind == FeatureCurveKind.Construction
    ...
    private static bool IsConstructionEligible(FeatureCurveKind kind)
    {
        return kind is FeatureCurveKind.Boundary or FeatureCurveKind.Crease or FeatureCurveKind.MaterialBoundary;
    }
    ...
    private static int HashSegmentId(int curveStableId, int segmentIndex)
    {
        unchecked
        {
            return (curveStableId * 397) ^ (segmentIndex + 1);
        }
    }
